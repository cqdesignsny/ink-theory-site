...
Premium tattoo studio website for **Ink Theory Tattoos**, led by Steven Martinez (6M+ social media followers). Studios in Montgomery, NY (Hudson Valley) and Dallas, TX (by appointment only).
...
- **Images**: 6 artist photos in `public/artists/`, 26 gallery images in `public/gallery/`, 12 body placement icons in `public/icons/`; shop images from `inktheorytattoos.com`; all optimized by Next.js (WebP/AVIF)
...
```
public/
  artists/              # 6 artist photos (local PNGs)
  gallery/              # 26 portfolio images (local)
  icons/                # 12 body placement PNG icons
  final-hero-vid.mp4    # Hero background video
src/
  app/
...
6 artists, all with locally stored photos in `public/artists/`:

- **Steven Martinez** (lead, Montgomery NY + Dallas TX)
- **Daron Fuller** (Montgomery NY)
- **Collin Hagerty** (Montgomery NY)
- **Adam Cordoba** (Montgomery NY)
- **Eric LeBron** (Montgomery NY)
- **Jazzy Camacho** (Montgomery NY)
...
| `/locations` | Static | Studio locations (Montgomery NY + Dallas TX by appointment only) |
...
**Total: 29 pages** (8 static + 19 SSG + not-found + sitemap)

## Locations

- **Montgomery, NY** (primary studio): 663 Route 17K, Montgomery, NY 12566. Open Mon thru Sat, 10 AM to 8 PM. Phone: (845) 385-2136
- **Dallas, TX** (by appointment only): Steven Martinez only. Private location shared after booking. Must book well in advance and inquire directly.
...
- **Video**: Hero background video (`final-hero-vid.mp4`) with deferred preload
...
Deployed on Vercel with automatic deploys from GitHub. The `next.config.ts` includes `remotePatterns` for the `inktheorytattoos.com` image domain.
